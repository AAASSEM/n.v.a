...
interface_patch = """        breakdown?: Array<{
            label: string;
            scope: number;
            tco2e: number;
            ef_used: number;
...
content = content.replace("""        breakdown?: Array<{
            label: string;
            scope: number;
            tco2e: number;
            ef_used: number;
        ...
        emirate?: string;""", interface_patch)

# 2. Add ElementBreakdownPanel before export default function StatDetailModal
panel_code = """
...
# 3. Add renderElementBreakdownSection before renderWaterBenchmark
...
call_patch = """                    {stat.scope === 2 && stat.breakdown && (
                        <Scope2CalculationPanel
                            breakdown={stat.breakdown}
                            emirate={stat.emirate}
                            gridEf={stat.grid_ef}
                            gridEfSource={stat.grid_ef_source}
                        />
                    )}
                    {renderElementBreakdownSection()}
                    {renderFuelBreakdown()}"""

content = content.replace("""                    {stat.scope === 2 && stat.breakdown && (
                        <Scope2CalculationPanel
                            breakdown={stat.breakdown}
                            emirate={stat.emirate}
                            gridEf={stat.grid_ef}
                    ...
                    {renderFuelBreakdown()}""", call_patch)
