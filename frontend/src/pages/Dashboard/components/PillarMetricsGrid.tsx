...
function asOfLabel(year: number, month: number, t: any): string {
    const key = `months.short.${MONTH_KEYS[month]}`;
    const { n } = useTranslation();
    return t('dash.asOf').replace('{{month}}', t(key)).replace('{{year}}', n(year));
}
            ...
            {quantitative.length > 0 && (
                <div>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: '#f0f2ff', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transform: isRtl ? 'scaleX(-1)' : 'none' }}>
                            <path d="M22 12h-4l-3 9L9 3l-3 9H2" />
                        </svg>
                        {t('dash.quantitativeMetrics')}
                    </h3>
                                    ...
                                    {el.year && el.month && (
                                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                            ...
                                            <span style={{
                                                ...
                                                {asOfLabel(el.year, el.month, t)}
                                            </span>
                                        </div>
                                    )}
                                </div>
                                <div style={{ fontSize: 13, color: '#8b90b8', marginBottom: 12, lineHeight: 1.4, minHeight: 36, textAlign: isRtl ? 'right' : 'left' }}>{t(el.name)}</div>
                                {el.value !== null ? (
                                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexDirection: 'row', justifyContent: isRtl ? 'flex-end' : 'flex-start' }}>
                                        <div style={{ fontSize: 24, fontWeight: 800, color: '#f0f2ff' }}>
                                            {n(el.value, { maximumFractionDigits: 1 })}
                                        </div>
                                        <div style={{ fontSize: 12, fontWeight: 600, color: color }}>
                                            {translateUnitStr(el.unit, t)}
                                        </div>
                                    </div>
                                ) : (
                                    <div style={{ fontSize: 14, color: '#475569', fontStyle: 'italic', fontWeight: 600, textAlign: isRtl ? 'right' : 'left' }}>{t('dash.pendingData')}</div>
                                )}
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
                        {policies.map(el => {
                            ...
                            if (isMultiSite) {
                                ...
                                text = t('dash.sitesCount').replace('{{count}}', n(yesCount)).replace('{{total}}', n(total));
                                ...
                                if (isYes) {
                                    ...
                                    badgeColor = '#10b981';
                                    icon = <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14M22 4L12 14.01l-3-3" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />;
                                    text = t('dash.implemented');
                                } else if (isNo) {
                                    badgeBg = 'rgba(244,63,94,0.1)';
                                    badgeColor = '#f43f5e';
                                    icon = <><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none" /><line x1="15" y1="9" x2="9" y2="15" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><line x1="9" y1="9" x2="15" y2="15" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></>;
                                    text = t('dash.notImplemented');
                                }
                            }
                            ...
                            return (
                                ...
                                >
                                    <div style={{ flex: 1, textAlign: isRtl ? 'right' : 'left' }}>
                                        ...
                                        <div style={{ fontSize: 13, color: '#f0f2ff', lineHeight: 1.4 }}>{t(el.name)}</div>
                                    </div>
                                    <div style={{ 
